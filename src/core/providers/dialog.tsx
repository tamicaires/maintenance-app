...
export const DialogProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getTopMostDialogId = useCallback(() => {
    return openDialogs.length > 0 ? openDialogs[openDialogs.length - 1].id : null
  }, [openDialogs])
  ...
  const openDialog = useCallback((options: DialogOptions) => {
    ...
    setDialogs((prevDialogs) => {
      ...
      if (options.stackable) {
        ...
        const filteredDialogs = updatedDialogs.filter((dialog) => dialog.isOpen)
        ...
        return (
          ...
          >
            ...
            >
              ...
              {dialog.lazyLoad ? (
                <Suspense fallback={<DialogLoadingFallback />}>{dialog.content}</Suspense>
              ) : (
                dialog.content
              )}
            </DialogContent>
